...
int main()
{
        ...
        iResult = getaddrinfo(HOST, DEFAULT_PORT, &hints, &result);   //инициализируем стурктуру для доступа к серверу
        if (iResult != 0) {
        ...
        HANDLE hParentStdIn = GetStdHandle(STD_INPUT_HANDLE);              // обработчик для stdin
        ...
        while (1)
        {
                ...
                iResult = send(client.socket, chBuf, dwRead, 0);   //клиент пишет серверу
